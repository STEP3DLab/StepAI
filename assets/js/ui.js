(function(){
  ...
  function renderProjects(){
    ...
    const res=(projects||[]).filter(p=>{
      const byTag=activeTag==='Все'||(p.tags||[]).includes(activeTag);
      const text=(p.title+' '+(p.description?.brief||'')+' '+(p.tags||[]).join(' ')).toLowerCase();
      const byQ=query?text.includes(query.toLowerCase()):true;
      return byTag&&byQ;
    });
    projEl.innerHTML=res.map(p=>`<div class="card project"><div class="img"><img loading="lazy" src="${p.image}" alt="${p.title}" style="width:100%;height:100%;object-fit:cover"/></div><div class="pad" style="padding:var(--space-6)"><div class="badges">${(p.tags||[]).map(t=>`<span class="badge">${t}</span>`).join('')}</div><h3 style="margin:10px 0 4px 0">${p.title}</h3><div class="soft" style="font-size:14px;display:flex;gap:var(--space-3);flex-wrap:wrap"><span>${p.tech}</span><span>${p.time}</span><span>${p.cost}</span></div><div style="margin-top:12px;display:flex;justify-content:flex-end"><button class="btn more" data-id="${p.id}">Подробно ▸</button></div></div></div>`).join('');
  }
  ...
  const modal=document.getElementById('modal');const modalBody=document.getElementById('modalBody');const modalClose=modal?.querySelector('.modal__close');let modalPrev=null;function openModal(id,src){if(!modal||!modalBody||!modalClose)return;modalPrev=src;const data=projects.find(p=>p.id===id);if(!data){toast('Данные недоступны');return;}modalBody.innerHTML=`<h3>${data.title}</h3><div class="soft" style="margin:8px 0;display:flex;gap:var(--space-3);flex-wrap:wrap;font-size:14px"><span>${data.tech}</span><span>${data.material}</span><span>${data.time}</span><span>${data.cost}</span></div><h4>ТЗ</h4><p>${data.description?.brief||''}</p><h4>Процесс</h4><p>${data.description?.process||''}</p><h4>Результат</h4><p>${data.description?.result||''}</p>${(data.description?.images||[]).map(src=>`<img loading="lazy" src="${src}" alt="${data.title}" style="margin-top:8px;max-width:100%"/>`).join('')}`;modal.hidden=false;setTimeout(()=>modalClose.focus(),0);document.addEventListener('keydown',modalKey);logger?.log('modal_open',{id});}
