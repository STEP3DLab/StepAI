(function(){
  ...
  // Telegram cover
  (function(){const a=document.getElementById('tgCover');if(!a)return;const svg=`<svg xmlns='http://www.w3.org/2000/svg' width='1280' height='640'><rect width='100%' height='100%' fill='white'/><g fill='none' stroke='black' opacity='0.06'>${Array.from({length:64}).map((_,i)=>`<line x1='${i*20}' y1='0' x2='${i*20}' y2='640'/>`).join('')}${Array.from({length:32}).map((_,i)=>`<line x1='0' y1='${i*20}' x2='1280' y2='${i*20}'/>`).join('')}</g><text x='50%' y='50%' font-family='Inter,Arial,Helvetica,sans-serif' font-size='144' font-weight='700' fill='black' text-anchor='middle' dominant-baseline='central'>Step3D</text><text x='50%' y='62%' font-family='Inter,Arial,Helvetica,sans-serif' font-size='28' fill='black' text-anchor='middle'>Агрегатор 3D‑услуг • step3d.tech</text><text x='50%' y='74%' font-family='Inter,Arial,Helvetica,sans-serif' font-size='20' fill='black' text-anchor='middle'>#мечтай #учись #твори #вдохновляй</text></svg>`;const blob=new Blob([svg],{type:'image/svg+xml'});a.href=URL.createObjectURL(blob);})();

  // Theme toggle
  const root=document.documentElement; const themeBtn=document.getElementById('theme'); const key='theme-step3d';
  function apply(mode){const sysDark=window.matchMedia('(prefers-color-scheme: dark)').matches;const useDark=mode==='auto'?sysDark:(mode==='dark');root.classList.toggle('dark',useDark);root.dataset.theme=mode;themeBtn.textContent=mode==='dark'?'☾':mode==='light'?'☼':'◐';themeBtn.setAttribute('aria-label','Тема: '+mode);updateThemeMeta();}
  ...
  function renderBlog(q=''){blogEl.innerHTML=(posts||[]).filter(p=>(p.title+' '+p.excerpt).toLowerCase().includes(q.toLowerCase())).map(p=>`<article class="post card pad"><div style="display:flex;align-items:center;justify-content:space-between"><time datetime="${p.date}">${p.date}</time><span class="badge">Новое</span></div><h3 style="margin:10px 0 4px 0">${p.title}</h3><div class="soft" style="font-size:14px">${p.excerpt}</div><div style="margin-top:10px"><a class="btn" href="#blog">Читать ▸</a></div></article>`).join('');}
  renderBlog();
  ...
  // Self-tests
  try{console.assert(document.querySelectorAll('.nav a.link').length>=6,'Nav links missing');console.assert(document.getElementById('kbarInput'),'kbar input missing');console.assert(document.getElementById('fx3d'),'fx3d missing');renderProjects();console.assert(document.getElementById('proj').children.length>=1,'Projects did not render');const before=price.textContent;document.getElementById('vol').value=String(+document.getElementById('vol').value+1);document.getElementById('vol').dispatchEvent(new Event('input'));console.assert(price.textContent!==before,'Estimator did not update');console.log('%cStep3D UI ready','color:green');}catch(e){console.warn('Self-tests failed:',e);}
})();
